...
const io = require('socket.io')({
  connectionStateRecovery: {
    ...
    maxDisconnectionDuration: 120 * 60 * 1000,
...
app.post('/endSession', async (req, res) => {
  ...
  console.log(
    'lemparannnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn req',
    req.body
  );
  // console.log("lemparannnnnnnnn res", res );
...
app.post('/client/rating', (req, res, next) => {
  ...
  const postData = {
    tenant: process.env.TENANT_ID,
    message: { rating, token, review_rating },
    // from action lempar clientRating jika dari agent jika tidak lempar clientRatingBot
    action: from === 'agent' ? 'clientRating' : 'clientRatingBot',
    dateSend: date.format(new Date(), 'YYYY-MM-DD HH:mm:ss'),
    user,
  };

  console.log('clientRating postData here');
  ...
  request(options, function (error, response) {
    try {
      ...
      if (response.statusCode >= 400) {
        return res
          .status(200)
          .json({ error: true, message: JSON.parse(response.body) });
      }
...
async function addChatHistory(
  ...
  console.log('addChatHistory caller====>', caller);
...
socketAuth(io, {
  authenticate: async (socket, data, callback) => {
    ...
    try {
      ...
      const canUpdate = await redis.setAsync(
        `livechat:${process.env.SESSION_KEY}:${user.token}`,
        socket.id,
        'XX',
        'EX',
